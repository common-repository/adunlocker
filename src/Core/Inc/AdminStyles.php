...
final class AdminStyles {
	...
	private function settings_styles() {
		...
		if ( in_array( $screen->base, Plugin::get_menu_bases(), true ) ) {
			...
			wp_enqueue_style( 'pp-AdUnlocker-ui', Plugin::get_url() . 'src/Core/Inc/assets/css/pp-ui.min.css', [], Plugin::get_version() );
            wp_enqueue_style( 'pp-AdUnlocker-unity-admin', Plugin::get_url() . 'src/Core/Inc/assets/css/admin' . Plugin::get_suffix() . '.css', [], Plugin::get_version() );
			wp_enqueue_style( 'pp-AdUnlocker-admin', Plugin::get_url() . 'css/admin' . Plugin::get_suffix() . '.css', [], Plugin::get_version() );
	...
	private function plugin_update_styles() {
		...
		if ( isset( $_GET['plugin'] ) && $_GET['plugin'] === 'AdUnlocker' ) {

			wp_enqueue_style( 'pp-AdUnlocker-plugin-install', Plugin::get_url() . 'src/Core/Inc/assets/css/plugin-install' . Plugin::get_suffix() . '.css', [], Plugin::get_version() );
