...
final class Elementor {
	...
	public function editor_styles() {
		...
		wp_enqueue_style( 'pp-AdUnlocker-admin', Plugin::get_url() . 'src/Core/Inc/assets/css/elementor-admin' . Plugin::get_suffix() . '.css', [], Plugin::get_version() );
	...
	public function register_widgets() {
		...
		$path = Plugin::get_path() . 'src/Core/AdUnlocker/Elementor/widgets/';
		...
		foreach ( new RecursiveIteratorIterator( new RecursiveDirectoryIterator( $path ) ) as $filename ) {
			...
			if ( substr( $filename, -4 ) === '.php' ) {
				...
				/** Add Namespace. */
                $widget_class = 'Core\AdUnlocker\Inc\\' . $widget_class;
	...
	private function sort_widgets() {
        ...
        foreach ( $widget_types as $key => $widget_type ) {
            ...
            /** Get widget order. */
	        $order = property_exists( $widget_type, 'pp_order' ) ? $widget_type->pp_order : 0;
