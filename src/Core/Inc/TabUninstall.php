...
final class TabUninstall extends Tab {
	...
	public function add_settings() {
        ...
        $group = 'AdUnlocker' . self::TAB_SLUG . 'OptionsGroup';
        $section = 'pp_AdUnlocker_' . self::TAB_SLUG . '_page_status_section';

		/** Delete plugin. */
		add_settings_field( 'delete_plugin', esc_html__( 'Removal settings:', 'AdUnlocker' ), [$this, 'render_delete_plugin'], $group, $section );
	...
	public function render_delete_plugin() {
		...
		/** Prepare description. */
		$helper_text = esc_html__( 'Choose which data to delete upon using the "Delete" action in the "Plugins" admin page.', 'AdUnlocker' );

		/** Render select. */
		UI::get_instance()->render_select(
			$options,
			$uninstall_settings[ 'delete_plugin' ], // Selected option.
			esc_html__( 'Delete plugin', 'AdUnlocker' ),
			$helper_text,
			[
			    'name' => 'pp_AdUnlocker_uninstall_settings[delete_plugin]'
            ]
		);
