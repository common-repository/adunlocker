...
final class TabStatus extends Tab {
	...
	public function render_reports() {
		...
		?>
		<div class="ppc-system-requirements">

			<?php foreach ( $reports as $key => $report ) : ?>
                <div class="pp-status-<?php esc_attr_e( $key ); ?>">
                    <table class="ppc-system-requirements-table">
                        <thead>
                            <tr>
	...
	private function render_string( $row ) {
        ...
        <tr>
            ...
            <td><span class="ppc-system-value"><?php echo wp_kses_post( $row['value'] ); ?></span></td>
            <th class="ppc-text-left">
                <?php if ( isset( $row['warning'] ) && $row['warning'] ) : ?>
                    <i class="material-icons ppc-system-warn">warning</i>
	...
	public function render_privacy_notice() {
        ...
        <div class="ppc-text-field-helper-line">
            <div class="ppc-text-field-helper-text ppc-text-field-helper-text--persistent"><?php
                esc_html_e( 'Some data will be sent to our server to verify purchase and to ensure that a plugin is compatible with your install. We will never collect any confidential data. All data is stored anonymously.', 'AdUnlocker' ); ?>
            </div>
        </div>
        <?php
