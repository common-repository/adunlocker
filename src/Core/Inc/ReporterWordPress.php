...
final class ReporterWordPress {
	...
	public function get_title() {
        ...
        return esc_html__( 'WordPress Environment', 'AdUnlocker' );
	...
	public function get_fields() {
		...
		return [
			'version'               => esc_html__( 'Version', 'AdUnlocker' ),
			'site_url'              => esc_html__( 'Site URL', 'AdUnlocker' ),
			'home_url'              => esc_html__( 'Home URL', 'AdUnlocker' ),
			'is_multisite'          => esc_html__( 'WP Multisite', 'AdUnlocker' ),
			'max_upload_size'       => esc_html__( 'Max Upload Size', 'AdUnlocker' ),
			'memory_limit'          => esc_html__( 'Memory limit', 'AdUnlocker' ),
			'permalink_structure'   => esc_html__( 'Permalink Structure', 'AdUnlocker' ),
			'language'              => esc_html__( 'Language', 'AdUnlocker' ),
			'timezone'              => esc_html__( 'Timezone', 'AdUnlocker' ),
			'admin_email'           => esc_html__( 'Admin Email', 'AdUnlocker' ),
			'debug_mode'            => esc_html__( 'Debug Mode', 'AdUnlocker' ),
		];
	...
	public function get_memory_limit() {
		...
		if ( $memory_limit_bytes < $min_recommended_bytes ) {
			$result['recommendation'] = esc_html__( 'We recommend setting memory to at least 64M. For more information, ask your hosting provider.', 'AdUnlocker' );
	...
	public function get_is_multisite() {
		...
		return [
			'value' => is_multisite() ? esc_html__( 'Yes', 'AdUnlocker' ) : esc_html__( 'No', 'AdUnlocker' )
		];
	...
	public function get_permalink_structure() {
		...
		if ( ! $structure ) {
			$structure = esc_html__( 'Plain', 'AdUnlocker' );
		}
	...
	public function get_debug_mode() {
		...
		return [
			'value' => WP_DEBUG ? esc_html__( 'Active', 'AdUnlocker' ) : esc_html__( 'Inactive', 'AdUnlocker' )
		];
